...
def detect_ocr(pix):
    langs = list(pd.read_csv('tessdata_urls.csv')['lang_code'].values)
    langs = '+'.join(langs)
    pix = cv2.imdecode(np.frombuffer(pix.tobytes('jpg'), np.uint8), -1)
    pix = cv2.resize(pix,None,fx=1.5,fy=1.5)
    pix = pix[..., ::-1]
    # text_list = []
    # text = pytesseract.image_to_string(pix, lang=langs, config='--oem 1')
    ...
    for block in text_blocks:
        segment_image = (block
                            ...
                            .crop_image(pix))
        text = pytesseract.image_to_string(segment_image, lang=langs, config='--oem 1')
        text_list.append(text)
    return '\n'.join(text_list)