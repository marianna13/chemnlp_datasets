...
if __name__=='__main__':
    ...
    pages = paginator.paginate(Bucket='medrxiv-src-monthly',RequestPayer='requester')

    s = time.time()
    

    for i, page in enumerate(pages):
        # if i < start*199//5 or i >= end*199//5:
        #     continue
    ...
    output_dir = 'medrxiv_parquets'
    os.makedirs(output_dir, exist_ok=True)
    files = glob.glob('medrxiv/**/*.txt', recursive=True)
    print(len(files))
    N = len(files)
    num_process = 20
    processes = []
